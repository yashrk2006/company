...
serve(async (req) => {
  try {
    ...
    if (table === 'project_inquiries') {
      subject = `[NEW LEAD] Zorvia Inquiry: ${record.name || 'Unknown Client'}`
      htmlContent = `
        <div style="font-family: Arial, sans-serif; background-color: #f8f9fa; padding: 20px;">
          <div style="max-w-xl p-8 bg-white border border-gray-200 rounded-xl shadow-lg">
            ...
            <p><strong>Email:</strong> ${record.email || 'N/A'} (Method: ${record.contact_method || 'Email'})</p>
            ...
            <p style="margin-top:30px; font-size: 12px; color: #9ca3af;">Please check the <a href="http://localhost:5173/zorvia-hq">Zorvia Command Center</a> for full details.</p>
          </div>
        </div>
      `
    } else if (table === 'strategic_updates') {
      subject = `[ZORVIA PING] Strategic Update from ${record.name || 'System'}`
      htmlContent = `
        <div style="font-family: Arial, sans-serif; padding: 20px;">
          <h2>Strategic Update Submitted</h2>
          <p><strong>Identifier:</strong> ${record.name}</p>
